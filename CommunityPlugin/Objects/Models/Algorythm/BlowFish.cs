...
{
    public class BlowFish : CSymmetricAlgorithm
    {
        private static readonly uint[] PBox = new uint[18]
        {
      608135816U,
      2242054355U,
      320440878U,
      ...
      2306472731U
        ...
        private static readonly uint[,] SBox = new uint[4, 256]
        {
      {
        3509652390U,
        2564797868U,
      ...
      },
      {
        ...
        private void PutBytes(byte[] buffer, byte[] data, int index)
        {
            for (int index1 = 0; index1 < data.Length; ++index1)
                buffer[index + index1] = data[index1];
        }

        private uint F(uint ui)
        {
            return (this._sBox[0, (int)(byte)(ui >> 24)] + this._sBox[1, (int)(byte)(ui >> 16)] ^ this._sBox[2, (int)(byte)(ui >> 8)]) + this._sBox[3, (int)(byte)ui];
        }
        ...
        protected override BlockCipher EncryptBlock(BlockCipher block)
        {
            ...
            uint ui1 = uint32_1 ^ this._pBox[0];
            uint ui2 = uint32_2 ^ (this.F(ui1) ^ this._pBox[1]);
            uint ui3 = ui1 ^ (this.F(ui2) ^ this._pBox[2]);
            uint ui4 = ui2 ^ (this.F(ui3) ^ this._pBox[3]);
            uint ui5 = ui3 ^ (this.F(ui4) ^ this._pBox[4]);
            uint ui6 = ui4 ^ (this.F(ui5) ^ this._pBox[5]);
            uint ui7 = ui5 ^ (this.F(ui6) ^ this._pBox[6]);
            uint ui8 = ui6 ^ (this.F(ui7) ^ this._pBox[7]);
            uint ui9 = ui7 ^ (this.F(ui8) ^ this._pBox[8]);
            uint ui10 = ui8 ^ (this.F(ui9) ^ this._pBox[9]);
            uint ui11 = ui9 ^ (this.F(ui10) ^ this._pBox[10]);
            uint ui12 = ui10 ^ (this.F(ui11) ^ this._pBox[11]);
            uint ui13 = ui11 ^ (this.F(ui12) ^ this._pBox[12]);
            uint ui14 = ui12 ^ (this.F(ui13) ^ this._pBox[13]);
            uint ui15 = ui13 ^ (this.F(ui14) ^ this._pBox[14]);
            uint ui16 = ui14 ^ (this.F(ui15) ^ this._pBox[15]);
            uint num = ui15 ^ (this.F(ui16) ^ this._pBox[16]);
            return new BlockCipher((DWord)(ui16 ^ this._pBox[17]), (DWord)num);
        }
